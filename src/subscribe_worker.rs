...
impl Worker {
    pub fn new(subrx: mpsc::Receiver<SubscribeReq>, pubrx: mpsc::Receiver<LogEntry>) -> Self {
        Worker { subrx, pubrx, subs: Vec::new() }
    }
            ...
            if let Ok(m) = self.subs[i].query.matches(&entry) {
                if m {
                    ...
                    match self.subs[i].res_tx.try_send(entry.clone()) {
                        ...
                        Err(TrySendError::Closed(_)) => { self.subs.remove(i); }
                    }
                }
            }
        }
...
mod tests {
    ...
    async fn test_matching_subscription() {
        ...
        let worker_handle = tokio::spawn(worker.run());
		{
			...
			let entry = LogEntry { msg: "This is a test message".to_string(), ..Default::default() };
        ...
        timeout(Duration::from_millis(500), worker_handle).await.unwrap().unwrap();
    }
    ...
    async fn test_non_matching_subscription() {
        ...
        let worker_handle = tokio::spawn(worker.run());
		{
			...
			subtx.send(req).await.unwrap();
			sleep(Duration::from_millis(100)).await;
			let entry = LogEntry { msg: "This message will not match".to_string(), ..Default::default() };
