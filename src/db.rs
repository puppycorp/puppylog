...
impl DB {
	...
	pub async fn fetch_bucket_logs(
		&self,
		bucket_ids: &[i32],
	) -> Result<Vec<(i32, BucketLogEntry)>> {
		if bucket_ids.is_empty() {
			return Ok(Vec::new());
		}
		let mut conn = self.read_conn()?;
		let rows: Vec<BucketLogRow> = bucket_logs::table
			.filter(bucket_logs::bucket_id.eq_any(bucket_ids))
			.order((
				bucket_logs::timestamp.desc(),
				bucket_logs::created_at.desc(),
				bucket_logs::id.desc(),
			))
			.load(&mut conn)?;
		...
		for row in rows {
			if let Some(entry) = Self::decode_bucket_log(&row) {
				entries.push((row.bucket_id, entry));
			}
		}
		Ok(entries)
	}
	...
	pub async fn list_bucket_queries(&self) -> Result<Vec<(i32, String)>> {
		let mut conn = self.read_conn()?;
		let rows: Vec<(i32, String)> = log_buckets::table
			.select((log_buckets::id, log_buckets::query))
			.load(&mut conn)?;
		Ok(rows)
	}
...
mod tests {
	...
	#[tokio::test]
	async fn fetch_bucket_logs_returns_requested_entries() {
		let db = test_db();
		let bucket_a = db
			.upsert_bucket(UpsertBucketArgs {
				id: None,
		...
		let bucket_b = db
			...
			.await
			.unwrap();
		let ts_a = Utc::now().to_rfc3339();
		let ts_b = (Utc::now() + chrono::Duration::seconds(1)).to_rfc3339();
		let entry_a = NewBucketLogEntry {
			id: "a-log".into(),
			timestamp: ts_a,
			level: "info".into(),
			msg: "bucket a".into(),
		...
		db.append_bucket_logs(bucket_a.id, &[entry_a.clone()])
			.await
			.unwrap();
		db.append_bucket_logs(bucket_b.id, &[entry_b.clone()])
			.await
			.unwrap();

		let only_a = db.fetch_bucket_logs(&[bucket_a.id]).await.unwrap();
		assert_eq!(only_a.len(), 1);
		assert_eq!(only_a[0].0, bucket_a.id);
		assert_eq!(only_a[0].1.id, entry_a.id);

		let both = db
			.fetch_bucket_logs(&[bucket_a.id, bucket_b.id])
			.await
			.unwrap();
		assert_eq!(both.len(), 2);
		let ids: HashSet<i32> = both.iter().map(|(bucket_id, _)| *bucket_id).collect();
		assert!(ids.contains(&bucket_a.id));
		assert!(ids.contains(&bucket_b.id));
	}
}
