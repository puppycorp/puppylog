...
var queriesPage = (root) => {
	...
	items.forEach((item) => {
		...
		row.onclick = () =>
			navigate(`/?query=${encodeURIComponent(item.query)}`)
...
class Histogram {
	...
	draw() {
		...
		const width = Math.max(
			this.canvas.parentElement?.clientWidth || 600,
...
var mainPage = (root) => {
	...
	logsSearchPage({
		...
		validateQuery: async (query2) => {
			let res = await fetch(
				`/api/v1/validate_query?query=${encodeURIComponent(query2)}`,
			)
...
var segmentsPage = async (root) => {
	const segementsMetadata = await fetch("/api/segment/metadata").then((res) =>
		res.json(),
	)
